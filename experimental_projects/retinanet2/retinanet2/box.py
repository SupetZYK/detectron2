...
class Anchors:
    def __init__(self, 
                sizes=[32, 64, 128, 256], # p3 -> p7
                ratios=[0.5, 1.0, 2.0], 
                scales=[1., pow(2,1/3.), pow(2,2/3.)],
                offset=0.0
                ):
        self.anchor_sizes = sizes
        self.anchor_areas = [s * s for s in self.anchor_sizes]  
        self.strides = [2 ** (i+3) for i in range(len(self.anchor_areas))] # stride of each fm, NOTE,  hard coded
        self.aspect_ratios = ratios
        self.scale_ratios = scales
        self.anchor_wh = self._get_anchor_wh()
        self.offset = offset
        self.num_anchors = len(ratios) * len(scales) # number anchors for each pixel
    ...
    def predict_transform(self, loc_preds, anchor_boxes, clip=True, bbx_reg_weight=[1, 1, 1, 1]):
        ...
        bbx_reg_weight = torch.Tensor(bbx_reg_weight).view(1, -1).to(loc_preds.device)
